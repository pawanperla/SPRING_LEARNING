...
When a bean is created and managed by the Spring container, it goes through several stages:

From creation → configuration → ready-to-use → destroy.

Phases of Bean Life Cycle

1. Bean Instantiation
   1. Spring creates the object of the bean using its constructor (either default or parameterized).
2. Dependency Injection (DI)
   1. Spring injects dependencies (e.g., via @Autowired, constructor, or setters).
   2. At this point, all required fields or collaborators are set.
3. @PostConstruct Method Call
   1. This is the custom init logic you define that should run after dependency injection.
4. afterPropertiesSet() - InitializingBean
   1. If your bean implements InitializingBean, Spring will call this method.
   2. Not commonly used these days (use @PostConstruct instead).
5. @PreDestroy Method Call
   1. Before the bean is destroyed, you can define custom logic using:
6. destroy() - DisposableBean
   1. If the bean implements DisposableBean, Spring will invoke this method when the container shuts down.
